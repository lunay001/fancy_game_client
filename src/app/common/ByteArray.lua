...
function ByteArray.leftToString(self, __radix, __separator)
    __radix = __radix or 16 
	__radix = ByteArray.radix[__radix] or "%02X"
	__separator = __separator or " "
	local __fmt = __radix..__separator
	...
	local __bytes = {}
    local size = #self._buf - self._pos + 1
	for i=1,size do
		__bytes[i] = __format(self._buf[self._pos+i-1])
	end
	return table.concat(__bytes) ,#__bytes
end
...
--[[
    从buf的当前指针开始读取一定长度的字节，放入目标ByteArray中
    @param __bytes 目标ByteArray
    @param __offset 目标ByteArray的偏移量
    @param __length 读取的字节长度
]] 
function ByteArray:readBytes(__bytes, __offset, __length)
	...
	__offset = __offset or 1
	if __offset > __selfLen then 
        __offset = 1 
    end

	__length = __length or 0
	if __length == 0 or __length > __availableLen then 
