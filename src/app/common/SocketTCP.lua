...
function SocketTCP:connect(__host, __port, __retryConnectWhenFailure)
	...
	--plog("%s.connect(%s, %d)", self.name, self.host, self.port)
	self.tcp = socket.tcp()
	...
	if not __checkConnect() then
		...
		local __connectTimeTick = function ()
			--plog("%s.connectTimeTick", self.name)
...
function SocketTCP:_onDisconnect()
	--plog("%s._onDisConnect", self.name);
	self.isConnected = false
	self:dispatchEvent({name=SocketTCP.EVENT_CLOSED})
	self:_reconnect();
end
...
function SocketTCP:_connectFailure(status)
	--plog("%s._connectFailure", self.name);
...
function SocketTCP:addEventListener(eventName, listener, tag)
    ...
    if ttag == "table" or ttag == "userdata" then
        plog("SocketTCP:addEventListener(eventName, listener, target) is deprecated, please use EventProtocol:addEventListener(eventName, handler(target, listener), tag)")
    ...
    if DEBUG > 1 then
        plog("%s [SocketTCP] addEventListener() - event: %s, handle: %s, tag: %s", tostring(self.target_), eventName, handle, tostring(tag))
    end
...
function SocketTCP:dispatchEvent(event)
    ...
    if DEBUG > 1 then
        -- plog("%s [SocketTCP] dispatchEvent() - event %s", tostring(self.target_), eventName)
    end
    ...
    for handle, listener in pairs(self.listeners_[eventName]) do
        if DEBUG > 1 then
            -- plog("%s [SocketTCP] dispatchEvent() - dispatching event %s to listener %s", tostring(self.target_), eventName, handle)
        end
        -- listener[1] = listener
        -- listener[2] = tag
        listener[1](event)
        if event.stop_ then
            if DEBUG > 1 then
                plog("%s [SocketTCP] dispatchEvent() - break dispatching for event %s", tostring(self.target_), eventName)
            end
            break
        end
    end
